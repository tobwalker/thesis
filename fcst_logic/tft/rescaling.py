...
def _parameter_robust(
    data: pl.DataFrame, column: str, upper_quantile: int, lower_quantile: int
) -> tuple[pl.DataFrame, pl.DataFrame]:
    return data.group_by(f"SERIES__{columns.OZ_ZSP}").agg(
        (pl.col(column).quantile(upper_quantile / 100) - pl.col(column).quantile(lower_quantile / 100)).alias("IQR"),
        pl.col(column).median().alias("MEDIAN"),
    )


def robust_applied_on_train_and_test(
    data: tuple[pl.DataFrame, pl.DataFrame], columns_to_rescale: list[str], upper_quantile: int, lower_quantile: int
) -> tuple[pl.DataFrame, pl.DataFrame, pl.DataFrame]:
    train_data = data[0]
    test_data = data[1]
    for column in columns_to_rescale:
        rescale_parameters = _parameter_robust(
            data=train_data, column=column, upper_quantile=upper_quantile, lower_quantile=lower_quantile
        )
        train_data = (
            train_data.join(rescale_parameters, on=f"SERIES__{columns.OZ_ZSP}", how="left")
            .with_columns(
        ...
        test_data = (
            test_data.join(rescale_parameters, on=f"SERIES__{columns.OZ_ZSP}", how="left")
            .with_columns(
                pl.when(pl.col(columns.CLUSTER) == 0).then(1).otherwise(pl.col("IQR")).alias("IQR"),
                pl.when(pl.col(columns.CLUSTER) == 0).then(0).otherwise(pl.col("MEDIAN")).alias("MEDIAN"),
            )
...
def between_0_and_1_on_train_and_test(
    ...
    for column in columns_to_rescale:
        ...
        train_data = (
            train_data.join(rescale_parameters, on=[f"SERIES__{columns.OZ_ZSP}", columns.CLUSTER], how="left")
            .with_columns(
                pl.when(pl.col("MAX_MIN_DISTANCE") == 0)
                .then(pl.col(column))
        ...
        test_data = (
            test_data.join(rescale_parameters, on=[f"SERIES__{columns.OZ_ZSP}", columns.CLUSTER], how="left")
            .with_columns(
                pl.when(pl.col("MAX_MIN_DISTANCE") == 0)
                .then(pl.col(column))
                .otherwise((pl.col(column) - pl.col("MIN")) / (pl.col("MAX_MIN_DISTANCE")))
